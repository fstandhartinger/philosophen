...
const ip=await ios.newPage();await ip.goto(base);await ip.getByRole('button',{name:'Ja',exact:true}).click();const iosText=await ip.locator('dialog').innerText();await ip.screenshot({path:'evidence/ios-install.png'});
